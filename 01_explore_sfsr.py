...
TESTLAGAR = [
    ...
    ("1998:808",  "Miljöbalk — stor lag, 173+ ändringar, många CELEX-nr"),
...
def _parse_ikraft(text: str) -> tuple[str, bool]:
    ...
    En övergångsbestämmelse (överg.best.) reglerar hur skiftet mellan gammal
    ...
    Exempel:
      "2005-04-01 överg.best." → ("2005-04-01", True)
      "2024-11-08"             → ("2024-11-08", False)
    """
    överg = "överg.best" in text
    datum = text.replace("överg.best.", "").strip()
    return datum, överg


def _parse_förarbeten(text: str) -> tuple[str, str, str]:
    """
...
def _etikett(sub_box) -> tuple[str, str]:
    """Returnerar (etikett, värde) för ett result-inner-sub-box-element."""
...
def parsa_sfsr(html: str) -> dict:
    ...
    for box in soup.select(".result-inner-box"):
        ...
        if "SFS-nummer" in full_text:
            ...
            meta["prop"], meta["bet"], meta["rskr"] = _parse_förarbeten(ft)
    ...
    for container in soup.select(".result-inner-sub-box-container"):
        ...
        for sub in container.select(".result-inner-sub-box"):
            etikett, värde = _etikett(sub)
            if etikett == "Rubrik":
                ändring["rubrik"] = värde
            elif etikett == "Omfattning":
                ändring["omfattning"] = värde
            elif etikett == "Ikraft":
                datum, överg = _parse_ikraft(värde)
                ändring["ikrafttradande"] = datum
                ändring["overgangsbestammelse"] = överg
            elif etikett == "Förarbeten":
                p, b, r = _parse_förarbeten(värde)
                ändring["prop"] = p
                ändring["bet"] = b
                ändring["rskr"] = r
            elif etikett == "CELEX-nr":
                ändring["celex"] = _parse_celex(värde)

        ändringar.append(ändring)

    meta["ändringar"] = ändringar
    return meta
...
def main() -> None:
  ...
  3. Ikraft-datum kan saknas (ändringar av ikraftträdandebestämmelser
     eller gamla ändringar med ofullständig registrering).

  4. "överg.best." i Ikraft-fältet signalerar en övergångsbestämmelse:
